...
-- foldl
reverseList5 :: [a] -> [a]
reverseList5 l = foldl (\acc e -> e : acc) [] l

-- foldr
reverseList6 :: [a] -> [a]
reverseList6 l = foldr (\e acc -> acc ++ [e]) [] l
...
removeDuplicatesRight l = (reverse (foldr (\x acc -> if(elem x acc) then acc else (acc ++ [x])) [] l))
