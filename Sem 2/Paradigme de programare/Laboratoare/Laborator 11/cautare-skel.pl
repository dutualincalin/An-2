...
parseState(_, _, _, _, _) :- false.
parseState(Mal/NM_E/NC_E/NM_V/NC_V, Mal, NM_E, NC_E, NM_V, NC_V).
...
%     pentru validare). Pentru a calcula numărul de canibali/zmisionari de pe malul opus
%     ce formulă puteți folosi știind ca numărul total de canibali/misionari este 3?


next_state(misionari, _, _) :- false.
next_state(misionari, est/NM_E/NC_E/NM_V/NC_V, vest/NM_E2/NC_E2/NM_V2/NC_V2):- 
	boat(NM_B, NC_B), NM_B =< NM_E, NC_B =< NC_E, 
	NM_E2 is NM_E - NM_B, NC_E2 is NC_E - NC_B,
	NM_V2 is NM_V + NM_B, NC_V2 is NC_V + NC_B,
	safeMisionari(NM_E2, NC_E2), safeMisionari(NM_V2, NC_V2).
next_state(misionari, vest/NM_E/NC_E/NM_V/NC_V, est/NM_E2/NC_E2/NM_V2/NC_V2):- 
	boat(NM_B, NC_B), NM_B =< NM_V, NC_B =< NC_V, 
	NM_V2 is NM_V - NM_B, NC_V2 is NC_V - NC_B,
	NM_E2 is NM_E + NM_B, NC_E2 is NC_E + NC_B,
	safeMisionari(NM_E2, NC_E2), safeMisionari(NM_V2, NC_V2).
...
bfs([(FiNd,Parent)|_], Closed, [(FiNd, Parent)|Closed]):- final_node(FiNd), !.
bfs([(CurrNode,_)|Rest], Closed, Sol):-  member((CurrNode,_), Closed), !, bfs(Rest, Closed, Sol).
bfs([(CurrNode,Parent)|Rest], Closed, Sol):- getNeighb(CurrNode, [], Kids), append(Rest, Kids, NewFrontier), bfs(NewFrontier, [(CurrNode,Parent)|Closed], Sol).
