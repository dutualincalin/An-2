...
permutari4([X,Y,Z,T], Perm) :- length(Perm, 4), member(X, Perm), member(Y, Perm), member(Z, Perm), member(T, Perm).
...
alternare(_, _, 0, []).
alternare(A, B, Len, [A | Res]) :- L is Len - 1, Len > 0, alternare(B, A, L, Res).
...
auCopil(A, B) :- parinte(C, A), parinte(C, B), A \= B, !.
...
soti(A, B) :- persoane(C), member(A, C), member(B, C), A @< B, auCopil(A, B).
