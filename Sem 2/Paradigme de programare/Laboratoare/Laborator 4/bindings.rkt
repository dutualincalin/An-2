...
(define (compute-f-with-step f a b step)
  (let iteration ((x b) (result '()))
    (if (> a x)
        result
        (iteration (- x step) (cons (f x) result)))))
...
(define (compute-perimeter points)
  (let ((start (car points)))
    (let iter ((points points) (perimeter 0))
      (if (null? (cdr points))
          (+ perimeter (distance (car points) start))
          (iter (cdr points) (+ perimeter (distance (car points) (cadr points))))))))
