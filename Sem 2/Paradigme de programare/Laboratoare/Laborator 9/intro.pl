...
myConcat([],L2,L2).
myConcat([E1|L1],L2,[E1|R]):- myConcat(L1, L2, R).
...
myReverse([], []).
myReverse([E1|L],R):- myReverse(L, RevL), myConcat(RevL, [E1], R).
...
myReverseAcc([], L, L).
myReverseAcc([E1 | List], Acc, RevList):- myReverseAcc(List, [E1 | Acc], RevList).
...
factorial(0 , 1).
factorial(N, Fact):- N > 0, Nn is N - 1, factorial(Nn, Factn), Fact is Factn * N.
