...
place player pos@(x, y) (Board b) = Board $ take n b ++ [cell] ++ drop (n + 1) b
    where
        n = 3 * x + y
        cell = if(player == 'X') then X else Zero
...
instance Show TicTacToe
    where
        show (Board b) = row1 ++ "\n" ++ row2 ++ "\n" ++ row3 where
            row1 = concatMap show (take 3 b)
            row2 = concatMap show (take 3 (drop 3 b))
            row3 = concatMap show (drop 6 b)
{-
...
instance Ord TicTacToe
    where
        (Board board1) <= (Board board2) = mutari board1 <= mutari board2 where
            mutari = length.filter (/= Blank)
