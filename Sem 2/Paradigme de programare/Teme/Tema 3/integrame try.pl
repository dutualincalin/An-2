...
next_state(PSols, (I, CurrentState), (NI, NextState)) :- 
	member((Q, []), CurrentState), !,
	member((Q, QS), PSols),
	member(LA, QS), atom_chars(A, LA), \+member((_, A), CurrentState),
	completare(I, [(Q, A)], NI), 
	check_integram(NI),
	write("NI: "), print_integrama(NI), nl, nl,
	set_NS(CurrentState, (Q, A), NextState).

final_state(State) :- \+member((_, []), State).

search(_, [(_, State)| _], State) :- final_state(State), !.
search(PSols, [CurrentState | Other], Solution) :-
	next_state(PSols, CurrentState, NextState),
	write("Next: "), write(CurrentState), nl, nl,
	\+member(CurrentState, Other),
	search(PSols, [NextState, CurrentState|Other], Solution).

rezolvare(I, Solutie) :- initial_state(I, State, PSols), search(PSols, [(I, State)], Solutie).