...
(define (rotate left right Acc)
  (if (and (stream-empty? left)(not (null? right))) (stream-cons (car right) Acc)
...
(define (enqueue x q)
  (if (< (queue-size-l q) (add1 (queue-size-r q))) 
    (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)] [right null] [size-l (add1 (+ (queue-size-l q) (queue-size-r q)))]  [size-r 0])
...
(define (dequeue q)
  (if (< (sub1 (queue-size-l q)) (queue-size-r q)) 
    (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)] [right null] [size-l (sub1 (+ (queue-size-l q) (queue-size-r q)))]  [size-r 0])
